...
- **Migrasi yang rebuild tabel** (pola `CREATE tabel_new → INSERT...SELECT → DROP tabel_lama → RENAME`, dipakai karena SQLite gak bisa `ALTER` constraint) **wajib** dibungkus `BEGIN TRANSACTION; ... COMMIT;` di sekeliling seluruh blok DDL-nya, dengan `PRAGMA foreign_keys = OFF/ON` di LUAR transaksi itu (SQLite gak izinkan pragma ini diubah di dalam transaksi terbuka). Lihat `007_kyai_kepala_asrama_pesan.sql` sebagai contoh pattern-nya. Tanpa ini, `wrangler d1 execute --file=` gak menjamin batch statement jalan atomik — proses yang keputus di tengah rebuild bisa ninggalin database tanpa tabel yang sedang di-rebuild sama sekali.
